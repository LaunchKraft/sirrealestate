...
export interface NcAddendumData {
...
export function generateNcAddendum(data: NcAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'North Carolina Due Diligence Request and Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('NORTH CAROLINA DUE DILIGENCE REQUEST AND AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on NC Realtors / NC Bar Association Form 310-T — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'The parties agree to modify the Offer to Purchase and Contract as follows. All other terms and conditions ' +
        'of the original agreement remain in full force and effect.',
        { lineGap: 2 },
      )
    doc.moveDown(0.4)

    sectionHeader(doc, '1. Agreed Repairs / Modifications')
    ...
    if (data.newClosingDate) {
      sectionHeader(doc, data.sellerCredit ? '3. Amended Settlement Date' : '2. Amended Settlement Date')
      row(doc, 'New Settlement Date', fmtDate(data.newClosingDate))
      doc.moveDown(0.3)
    }
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the Offer to Purchase and Contract remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the NC Realtors / NC Bar Association Form 310-T structure. ' +
