...
export interface WaAddendumData {
...
export function generateWaAddendum(data: WaAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Washington Inspection Response / Addendum' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('WASHINGTON INSPECTION RESPONSE / ADDENDUM', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on NWMLS Inspection Response Form — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'The parties agree to modify the Purchase and Sale Agreement as follows. All other terms and conditions ' +
        'of the original agreement remain in full force and effect. All deadlines are measured from the ' +
    ...
    if (data.newClosingDate) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'The closing date in the original Purchase and Sale Agreement is hereby amended to the date shown above.',
          60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
        )
      doc.moveDown(0.3)
    }
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the Purchase and Sale Agreement remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on NWMLS Inspection Response form structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official NWMLS form for your signature. ' +
        'Both buyer and seller must sign addenda for them to be binding. ' +
