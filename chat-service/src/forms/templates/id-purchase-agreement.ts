...
export interface IdPurchaseAgreementData {
...
export function generateIdPurchaseAgreement(data: IdPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Idaho Real Estate Purchase and Sale Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('IDAHO REAL ESTATE PURCHASE AND SALE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on IREC Purchase and Sale Agreement — For Educational/Reference Purposes. ' +
        'Your agent will complete the official IREC form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        ...
        'the agreement within this period. Seller has 5 business days to respond to any inspection requests.',
    ...
    // ── Seller Disclosure ─────────────────────────────────────────────────
    sectionHeader(doc, 'Seller Property Condition Disclosure')
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per Idaho Code § 55-2501, the Seller is required to provide a Property Condition Disclosure form ' +
        'disclosing material defects known to the Seller. Buyer should review this disclosure and raise any ' +
        'concerns during the Inspection Period.',
      )
    doc.moveDown(0.3)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the IREC Purchase and Sale Agreement structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official IREC form for your signature. ' +
        'Idaho law requires real estate licensees to use approved forms. ' +
