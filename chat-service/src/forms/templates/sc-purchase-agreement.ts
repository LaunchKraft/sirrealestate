...
export interface ScPurchaseAgreementData {
  ...
  /** Closing attorney — required in South Carolina */
...
export function generateScPurchaseAgreement(data: ScPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'South Carolina Offer to Purchase and Contract of Sale' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('SOUTH CAROLINA OFFER TO PURCHASE AND CONTRACT OF SALE', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on SCR Form 400 — For Educational/Reference Purposes. ' +
        'Your agent will complete the official SCR form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    row(doc, 'Held By', data.closingAttorney ?? 'Closing Attorney / Escrow Agent (to be designated)')
    row(doc, 'EMD Deadline', 'Within 5 days of acceptance')
    doc.moveDown(0.2)
    doc.fontSize(8).font('Helvetica').fillColor(DARK)
      .text(
        'South Carolina requires that all residential real estate closings be conducted by a licensed SC attorney. ' +
        'Earnest money is typically held by the closing attorney.',
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        `Buyer has ${data.inspectionDays} business days from acceptance to conduct all desired inspections. ` +
        'Buyer may request repairs or credits, or terminate the agreement within this period.',
        60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
      )
    doc.moveDown(0.4)
    ...
    // ── 9. Closing Attorney (SC-specific) ─────────────────────────────────
    sectionHeader(doc, '9. Closing Attorney (Required in South Carolina)')
    row(doc, 'Closing Attorney / Firm', data.closingAttorney ?? 'To be designated by Buyer')
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'South Carolina law requires that residential real estate closings be conducted by a licensed SC attorney. ' +
        'The buyer typically selects the closing attorney. The attorney will conduct the title search, ' +
        'prepare closing documents, and disburse funds.',
      )
    doc.moveDown(0.3)
    ...
    // ── Seller Disclosure ─────────────────────────────────────────────────
    sectionHeader(doc, 'Residential Property Condition Disclosure')
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per SC Code § 27-50-10, the Seller is required to provide a Residential Property Condition Disclosure Statement. ' +
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the SCR Form 400 structure. ' +
