...
export interface OrAddendumData {
...
export function generateOrAddendum(data: OrAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Oregon Repair/Remedy Addendum' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('OREGON REPAIR/REMEDY ADDENDUM', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on Oregon Realtors OREF Addendum — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'The parties agree to modify the Sale Agreement as follows. All other terms and conditions ' +
        'of the original agreement remain in full force and effect.',
        { lineGap: 2 },
      )
    doc.moveDown(0.4)

    sectionHeader(doc, '1. Agreed Repairs / Remedies')
    ...
    if (data.newClosingDate) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'The closing date in the original Sale Agreement is hereby amended to the date shown above.',
          60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
        )
      doc.moveDown(0.3)
    }
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the Sale Agreement remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the Oregon Realtors OREF Addendum structure. ' +
