...
export interface NvAddendumData {
  generatedDate: string
  listingAddress: string
  originalContractDate: string
  buyers: Array<{ fullLegalName: string }>
  amendments: Array<{
    description: string   // e.g. "Seller to repair roof by closing", "Seller to provide $3,000 credit at closing"
  }>
  sellerCredit?: number   // total seller credit if any
  newClosingDate?: string // if closing date is being amended
}
...
export function generateNvAddendum(data: NvAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'NVAR Addendum to Purchase Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('NEVADA ADDENDUM TO PURCHASE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on NVAR Addendum Form — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    if (data.newClosingDate) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'The closing date in the original Purchase Agreement is hereby amended to the date shown above.',
          60,
          doc.y,
          { width: doc.page.width - 120, lineGap: 2 },
        )
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the Purchase Agreement remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the NVAR Addendum form structure. ' +
