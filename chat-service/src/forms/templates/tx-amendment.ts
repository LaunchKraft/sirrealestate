...
export interface TxAmendmentData {
  generatedDate: string
  listingAddress: string
  originalContractDate: string
  buyers: Array<{ fullLegalName: string }>
  amendments: Array<{
    description: string    // e.g. "Seller to repair roof by closing", "Seller to provide $2,500 credit at closing"
  }>
...
export function generateTxAmendment(data: TxAmendmentData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'TREC Amendment to Contract' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('TREC AMENDMENT TO CONTRACT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on TREC Form 39-8 — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    if (data.newClosingDate) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'The closing date in the original contract is hereby amended to the date shown above.',
          60,
          doc.y,
          { width: doc.page.width - 120, lineGap: 2 },
        )
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the contract remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on TREC Form 39-8 structure. ' +
