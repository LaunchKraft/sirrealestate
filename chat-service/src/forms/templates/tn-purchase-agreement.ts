...
export interface TnPurchaseAgreementData {
...
export function generateTnPurchaseAgreement(data: TnPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Tennessee Residential Purchase and Sale Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('TENNESSEE RESIDENTIAL PURCHASE AND SALE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on Tennessee Realtors Form — For Educational/Reference Purposes. ' +
        'Your agent will complete the official Tennessee Realtors form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    row(doc, 'Held By', data.titleCompany ?? 'Title Company / Escrow Agent (to be designated)')
    ...
    if (data.titleCompany) {
      ...
      row(doc, 'Title / Closing Agent', data.titleCompany)
    }
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per TCA § 66-5-201, the Seller is required to provide a Residential Property Condition Disclosure. ' +
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the Tennessee Realtors Purchase and Sale Agreement structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official Tennessee Realtors form for your signature. ' +
