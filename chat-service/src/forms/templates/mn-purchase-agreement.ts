...
export interface MnPurchaseAgreementData {
...
export function generateMnPurchaseAgreement(data: MnPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Minnesota Purchase Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('MINNESOTA PURCHASE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on Minnesota Realtors Purchase Agreement — For Educational/Reference Purposes. ' +
        'Your agent will complete the official Minnesota Realtors form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    row(doc, 'Held By', data.titleCompany ?? 'Title Company (to be designated)')
    row(doc, 'EMD Deadline', 'Upon acceptance (same day or next business day)')
    ...
    sectionHeader(doc, '6. Inspection Contingency')
    row(doc, 'Inspection Period', `${data.inspectionDays} business days from acceptance`)
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        `Buyer has ${data.inspectionDays} business days from the date of acceptance to complete all desired inspections. ` +
        'Buyer may request repairs or a price reduction, or may cancel the agreement within this period. ' +
        'Minnesota Realtors forms include a separate Inspection Contingency Addendum.',
    ...
    row(doc, 'Date of Closing', fmtDate(data.closingDate))
    row(doc, 'Possession Date', data.possessionDate ? fmtDate(data.possessionDate) : 'Date of Closing')
    doc.moveDown(0.2)
    doc.fontSize(8).font('Helvetica').fillColor(MUTED)
      .text(
        'Note: Buyer has the right to a walk-through of the property within 24 hours before closing to confirm condition.',
        60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
      )
    doc.moveDown(0.3)
    ...
    // ── HOA ───────────────────────────────────────────────────────────────
    if (data.hasHoa) {
      sectionHeader(doc, 'HOA / Common Interest Community')
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'This property is subject to a Homeowners Association or Common Interest Community. ' +
          'Per Minnesota Statute § 515B.4-107, Seller must provide HOA documents, bylaws, financials, and the resale disclosure certificate. ' +
          'Buyer has 10 days after receipt to cancel based on the HOA documents.',
        )
      doc.moveDown(0.3)
    }
    ...
    if (data.titleCompany) {
      sectionHeader(doc, 'Title Company')
    ...
    if ((data.inclusions && data.inclusions.length > 0) || (data.exclusions && data.exclusions.length > 0)) {
      sectionHeader(doc, 'Personal Property / Inclusions & Exclusions')
    ...
    // ── Seller Disclosure ─────────────────────────────────────────────────
    sectionHeader(doc, "Seller's Property Disclosure Statement")
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per Minnesota Statute § 513.55, the Seller is required to provide a Seller\'s Property Disclosure Statement ' +
        'disclosing all known material facts. Buyer should review the disclosure and raise any concerns during the inspection period. ' +
        'Note: Properties in Minneapolis may also be subject to the local Truth-in-Housing ordinance.',
      )
    doc.moveDown(0.3)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the Minnesota Realtors Purchase Agreement structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official Minnesota Realtors form for your signature. ' +
