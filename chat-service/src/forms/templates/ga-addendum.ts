...
export interface GaAddendumData {
...
export function generateGaAddendum(data: GaAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Georgia Amendment to Purchase and Sale Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('GEORGIA AMENDMENT TO PURCHASE AND SALE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on GAR Amendment Form — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.moveDown(0.3)
    doc.font('Helvetica-Bold').text('Original Contract Date (Binding Agreement Date):  ', { continued: true })
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the GAR Amendment form structure. ' +
