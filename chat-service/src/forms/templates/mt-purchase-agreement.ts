...
export interface MtPurchaseAgreementData {
  ...
  hasHoa: boolean
  hasWaterRights?: boolean         // Montana-specific: water rights disclosure
...
export function generateMtPurchaseAgreement(data: MtPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Montana Buy-Sell Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('MONTANA RESIDENTIAL BUY-SELL AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on MAR Buy-Sell Agreement — For Educational/Reference Purposes. ' +
        'Your agent will complete the official MAR form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    // ── 9. Water Rights Disclosure (Montana-specific) ─────────────────────
    sectionHeader(doc, '9. Water Rights Disclosure')
    if (data.hasWaterRights) {
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'This property includes water rights. Per Montana law, the Seller must provide a Water Rights ' +
          'Disclosure. Buyer should review the water rights documentation during the inspection period. ' +
          'Water rights in Montana are governed by the prior appropriation doctrine (first in time, first in right).',
          60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
        )
    } else {
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'Seller represents that there are no water rights associated with this property, or that such rights ' +
          'are not being conveyed with the sale. Buyer should confirm during the inspection period.',
          60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
        )
    }
    doc.moveDown(0.4)
    ...
    if (data.titleCompany) {
      sectionHeader(doc, '10. Title & Escrow')
    ...
    if (data.hasHoa) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          ...
          'Seller must provide all HOA documents, bylaws, and financial statements. ' +
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per MCA § 37-51-313, the Seller is required to provide a Seller\'s Property Disclosure Statement ' +
        'disclosing known material defects. Buyer should review the disclosure and raise any concerns during ' +
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the MAR Buy-Sell Agreement structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official MAR form for your signature. ' +
