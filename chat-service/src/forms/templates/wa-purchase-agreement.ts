...
export interface WaPurchaseAgreementData {
  generatedDate: string
  listingAddress: string
  purchasePrice: number
  earnestMoneyAmount: number
  closingDate: string
  /** Mutual acceptance date — Washington-specific term for when both parties have signed */
...
export function generateWaPurchaseAgreement(data: WaPurchaseAgreementData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Washington Residential Purchase and Sale Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('WASHINGTON RESIDENTIAL PURCHASE AND SALE AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text(
        'Based on NWMLS Form 21 — For Educational/Reference Purposes. ' +
        'Your agent will complete the official NWMLS form.',
        { align: 'center' },
      )
    doc.moveDown(0.3)
    ...
    if (data.mutualAcceptanceDate) {
      row(doc, 'Mutual Acceptance Date', fmtDate(data.mutualAcceptanceDate))
    }
    ...
    row(doc, 'EMD Deadline', 'Within 2 business days of mutual acceptance')
    doc.moveDown(0.2)
    doc.fontSize(8).font('Helvetica').fillColor(DARK)
      .text(
        'In Washington, the transaction clock begins at "Mutual Acceptance" — the date the last party signs ' +
        'and communicates acceptance to the other party. All deadlines run from this date.',
    ...
    row(doc, 'Inspection Period', `${data.inspectionDays} business days from mutual acceptance`)
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        `Buyer has ${data.inspectionDays} business days from mutual acceptance to conduct all desired ` +
        'inspections. Buyer may request repairs or credits via an Inspection Response addendum, or may ' +
        'terminate the agreement. All inspection contingency deadlines run from the mutual acceptance date.',
        60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
      )
    doc.moveDown(0.4)
    ...
    checklist(doc, [
      {
        ...
        note: data.contingencies.inspection ? `${data.inspectionDays} business days from mutual acceptance` : undefined,
      },
    ...
    if (data.hasHoa) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          ...
          'Seller must provide all HOA documents, resale certificate, and financial statements. ' +
    ...
    sectionHeader(doc, 'Seller Disclosure Statement (Form 17)')
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'Per RCW 64.06.013, the Seller is required to provide a Seller Disclosure Statement (NWMLS Form 17). ' +
        'This disclosure is required by Washington law and must be provided before or within 5 business days ' +
        'of mutual acceptance. Buyer has 3 business days after receipt to revoke the agreement based on the disclosure.',
      )
    doc.moveDown(0.3)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on NWMLS Form 21 structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official NWMLS form for your signature. ' +
        'Washington law requires real estate licensees to use approved NWMLS forms. ' +
