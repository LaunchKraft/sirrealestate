...
export interface MtAddendumData {
...
export function generateMtAddendum(data: MtAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Montana Addendum to Buy-Sell Agreement' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('MONTANA ADDENDUM TO BUY-SELL AGREEMENT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on MAR Addendum Form — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'The parties agree to modify the Buy-Sell Agreement as follows. All other terms and conditions ' +
    ...
    if (data.newClosingDate) {
      ...
      doc.fontSize(9).font('Helvetica').fillColor(DARK)
        .text(
          'The closing date in the original Buy-Sell Agreement is hereby amended to the date shown above.',
          60, doc.y, { width: doc.page.width - 120, lineGap: 2 },
        )
      doc.moveDown(0.3)
    }
    ...
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK)
      .text('All other terms of the Buy-Sell Agreement remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the MAR Addendum form structure. ' +
