...
export interface FlAddendumData {
...
export function generateFlAddendum(data: FlAddendumData): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    const doc = new PDFDocument({ size: 'LETTER', margin: 50, info: { Title: 'Florida Addendum to Contract' } })
    ...
    doc.fontSize(15).font('Helvetica-Bold').fillColor(DARK)
      .text('FLORIDA ADDENDUM TO CONTRACT', { align: 'center' })
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica').fillColor(MUTED)
      .text('Based on Florida Realtors / Florida Bar Addendum — For Educational/Reference Purposes', { align: 'center' })
    doc.moveDown(0.3)
    ...
    doc.moveDown(0.3)
    doc.font('Helvetica-Bold').text('Original Contract Date (Effective Date):  ', { continued: true })
    ...
    doc.fontSize(9).font('Helvetica').fillColor(DARK)
      .text(
        'The parties agree to modify the Contract for Residential Sale and Purchase as follows. ' +
    ...
    rule(doc)
    doc.moveDown(0.3)
    doc.fontSize(9).font('Helvetica-Bold').fillColor(DARK).text('All other terms of the Contract remain unchanged.')
    doc.moveDown(0.5)
    ...
    doc.fontSize(7).fillColor(MUTED)
      .text(
        'IMPORTANT: This is a system-generated summary for reference purposes only, based on the Florida Realtors / Florida Bar Addendum structure. ' +
        'It is not a legally binding document. Your real estate agent will prepare the official Florida form for your signature. ' +
