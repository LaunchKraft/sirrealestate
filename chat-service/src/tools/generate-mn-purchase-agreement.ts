...
import { generateMnPurchaseAgreement } from '../forms/templates/mn-purchase-agreement'
...
export const definition = {
  name: 'generate_mn_purchase_agreement',
  description:
    'Generate the Minnesota Purchase Agreement for a Minnesota property purchase. ' +
    'Call this after offer terms are finalized for MN properties. ' +
    'EMD is due upon acceptance (same day or next business day). ' +
    'Buyer has the right to a walk-through within 24 hours before closing. ' +
...
interface GenerateMnPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateMnPurchaseAgreementInput,
  ...
  const pdfBuffer = await generateMnPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `mn-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Minnesota Purchase Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    title,
    subject: title,
    message: 'Your Minnesota Purchase Agreement is ready for review and signature. Please sign before submitting your offer to the seller\'s agent.',
    signers,
  ...
  return {
    documentId,
    message:
      `Minnesota Purchase Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
      'Earnest money is due upon acceptance (same day or next business day). ' +
