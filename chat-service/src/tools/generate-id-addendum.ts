...
import { generateIdAddendum } from '../forms/templates/id-addendum'
...
export const definition = {
  name: 'generate_id_addendum',
  description:
    'Generate an Addendum to the Idaho Purchase and Sale Agreement. ' +
...
interface GenerateIdAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateIdAddendum(data)

  const documentId = randomUUID()
  const fileName = `id-addendum-${closing.closingId}.pdf`
  ...
  const title = `Idaho Addendum to Purchase and Sale Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Idaho Addendum to Purchase and Sale Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Idaho Addendum to Purchase and Sale Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
