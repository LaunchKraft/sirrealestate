...
import { generateTnPurchaseAgreement } from '../forms/templates/tn-purchase-agreement'
...
export const definition = {
  name: 'generate_tn_purchase_agreement',
  description:
    'Generate the Tennessee Residential Purchase and Sale Agreement for a Tennessee property purchase. ' +
    'Call this after offer terms are finalized for TN properties. ' +
...
interface GenerateTnPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateTnPurchaseAgreementInput,
  ...
  const pdfBuffer = await generateTnPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `tn-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Tennessee Purchase and Sale Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Tennessee Residential Purchase and Sale Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Tennessee Purchase and Sale Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
