...
import { generateAzRpc } from '../forms/templates/az-rpc'
...
export const definition = {
  name: 'generate_az_rpc',
  description:
    'Generate the AAR Residential Purchase Contract (RPC) for an Arizona property purchase. ' +
    'Call this after offer terms are finalized for AZ properties.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      inspectionPeriodDays: {
        type: 'number',
        description: 'Inspection period in days (default 10).',
      },
    },
    required: ['closingId'],
  },
...
export async function execute(
  ...
  const data = {
    ...
    inspectionPeriodDays: input.inspectionPeriodDays ?? 10,
  ...
  const pdfBuffer = await generateAzRpc(data)

  const documentId = randomUUID()
  const fileName = `az-rpc-${closing.closingId}.pdf`
  ...
  const title = `Arizona Purchase Contract — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    title,
    subject: title,
    message: 'Your Arizona Residential Purchase Contract is ready for review and signature. Please sign before submitting your offer.',
    signers,
  ...
  return {
    documentId,
    message: `Arizona Residential Purchase Contract generated and sent to ${buyerEmails} for signing via Dropbox Sign. Please sign before submitting the offer to the seller's agent.`,
  }
}
