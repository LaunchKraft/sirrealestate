...
import { generateTnAddendum } from '../forms/templates/tn-addendum'
...
export const definition = {
  name: 'generate_tn_addendum',
  description:
    'Generate a Tennessee Amendment to the Purchase and Sale Agreement to document agreed repairs, credits, or contract modifications. ' +
...
interface GenerateTnAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateTnAddendum(data)
  const documentId = randomUUID()
  const fileName = `tn-addendum-${closing.closingId}.pdf`
  ...
  const title = `Tennessee Amendment to Purchase and Sale Agreement — ${closing.listingAddress}`
  const { signatureRequestId } = await signer.createSignatureRequest({
    pdfBuffer, fileName, title, subject: title,
    message: 'Your Tennessee Amendment to Purchase and Sale Agreement is ready for review and signature. After you sign, the seller must countersign.',
    signers,
  ...
  return {
    documentId,
    message: `Tennessee Amendment generated and sent to ${offer.buyers.map((b) => b.email).join(', ')} for signing. After buyer signs, seller must countersign.`,
  }
}
