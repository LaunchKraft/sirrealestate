...
import { generateGaAddendum } from '../forms/templates/ga-addendum'
...
export const definition = {
  name: 'generate_ga_addendum',
  description:
    'Generate a Georgia Amendment to the Purchase and Sale Agreement to document agreed repairs, credits, or contract modifications. ' +
    'Use this during or after the Due Diligence Period. Both buyer and seller must sign.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      amendments: {
        type: 'array',
        description: 'List of agreed amendments (minimum 1 item).',
        items: {
          type: 'object',
          properties: { description: { type: 'string', description: 'Description of the amendment.' } },
          required: ['description'],
        },
        minItems: 1,
      },
      sellerCredit: { type: 'number', description: 'Total seller credit amount in dollars, if applicable.' },
      newClosingDate: { type: 'string', description: 'New closing date in ISO format (YYYY-MM-DD) if being amended.' },
    },
...
interface GenerateGaAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateGaAddendum(data)
  const documentId = randomUUID()
  const fileName = `ga-addendum-${closing.closingId}.pdf`
  ...
  const title = `Georgia Amendment to Purchase and Sale Agreement — ${closing.listingAddress}`
  const { signatureRequestId } = await signer.createSignatureRequest({
    pdfBuffer, fileName, title, subject: title,
    message: 'Your Georgia Amendment to Purchase and Sale Agreement is ready for review and signature. After you sign, the seller must countersign.',
    signers,
  ...
  return {
    documentId,
    message: `Georgia Amendment generated and sent to ${offer.buyers.map((b) => b.email).join(', ')} for signing. After buyer signs, seller must countersign.`,
  }
}
