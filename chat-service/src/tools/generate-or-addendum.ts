...
import { generateOrAddendum } from '../forms/templates/or-addendum'
...
export const definition = {
  name: 'generate_or_addendum',
  description:
    'Generate an Oregon Repair/Remedy Addendum to the Sale Agreement. ' +
  ...
  input_schema: {
    ...
    properties: {
      ...
      amendments: {
        type: 'array',
        description: 'List of agreed repairs/remedies (minimum 1 item).',
        items: {
          type: 'object',
          properties: {
            description: {
              type: 'string',
              description: 'Description of the repair or remedy, e.g. "Seller to repair roof by closing" or "Seller to provide $3,000 credit at closing".',
            },
          },
...
interface GenerateOrAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateOrAddendum(data)

  const documentId = randomUUID()
  const fileName = `or-addendum-${closing.closingId}.pdf`
  ...
  const title = `Oregon Repair/Remedy Addendum — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Oregon Repair/Remedy Addendum is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Oregon Repair/Remedy Addendum generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
