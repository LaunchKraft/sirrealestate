...
import { generateFlAddendum } from '../forms/templates/fl-addendum'
...
export const definition = {
  name: 'generate_fl_addendum',
  description:
    'Generate a Florida Addendum to Contract to document agreed repairs, credits, or contract modifications. ' +
...
interface GenerateFlAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateFlAddendum(data)
  const documentId = randomUUID()
  const fileName = `fl-addendum-${closing.closingId}.pdf`
  ...
  const title = `Florida Addendum to Contract — ${closing.listingAddress}`
  const { signatureRequestId } = await signer.createSignatureRequest({
    pdfBuffer, fileName, title, subject: title,
    message: 'Your Florida Addendum to Contract is ready for review and signature. After you sign, the seller must countersign.',
    signers,
  ...
  return {
    documentId,
    message: `Florida Addendum to Contract generated and sent to ${offer.buyers.map((b) => b.email).join(', ')} for signing. After buyer signs, seller must countersign.`,
  }
}
