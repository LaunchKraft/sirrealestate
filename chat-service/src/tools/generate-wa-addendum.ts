...
import { generateWaAddendum } from '../forms/templates/wa-addendum'
...
export const definition = {
  name: 'generate_wa_addendum',
  description:
    'Generate a Washington Inspection Response / Addendum to the Purchase and Sale Agreement. ' +
    'Use this during or after the inspection period to document agreed repairs, seller credits, or other contract modifications. ' +
    'Both buyer and seller must sign. All deadlines in Washington run from the mutual acceptance date.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      amendments: {
        ...
        items: {
          ...
          properties: {
            description: {
              type: 'string',
              description: 'Description of the modification, e.g. "Seller to repair HVAC by closing" or "Seller to provide $5,000 credit at closing".',
            },
          },
...
interface GenerateWaAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateWaAddendum(data)

  const documentId = randomUUID()
  const fileName = `wa-addendum-${closing.closingId}.pdf`
  ...
  const title = `Washington Inspection Response / Addendum — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Washington Inspection Response / Addendum is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Washington Inspection Response / Addendum generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
