...
import { generateScAddendum } from '../forms/templates/sc-addendum'
...
export const definition = {
  name: 'generate_sc_addendum',
  description:
    'Generate a South Carolina Amendment to the Contract of Sale to document agreed repairs, credits, or contract modifications. ' +
    'Use this during or after the inspection period. Both buyer and seller must sign. ' +
    'South Carolina closings must be conducted by a licensed SC attorney.',
  input_schema: {
    type: 'object',
    properties: {
...
interface GenerateScAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateScAddendum(data)
  const documentId = randomUUID()
  const fileName = `sc-addendum-${closing.closingId}.pdf`
  ...
  const title = `SC Amendment to Contract of Sale — ${closing.listingAddress}`
  const { signatureRequestId } = await signer.createSignatureRequest({
    pdfBuffer, fileName, title, subject: title,
    message: 'Your South Carolina Amendment to Contract of Sale is ready for review and signature. After you sign, the seller must countersign.',
    signers,
  ...
  return {
    documentId,
    message: `SC Amendment to Contract of Sale generated and sent to ${offer.buyers.map((b) => b.email).join(', ')} for signing. After buyer signs, seller must countersign.`,
  }
}
