...
import { generateNvPurchaseAgreement } from '../forms/templates/nv-purchase-agreement'
...
export const definition = {
  name: 'generate_nv_purchase_agreement',
  description:
    'Generate the NVAR Residential Purchase Agreement for a Nevada property purchase. ' +
    'Call this after offer terms are finalized for NV properties. ' +
    'The Due Diligence Period gives the buyer an unconditional right to cancel — always confirm dueDiligenceDays with the buyer.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      dueDiligenceDays: {
        type: 'number',
        description: 'Number of calendar days for the Due Diligence Period (default 10). Typically 10–15 days.',
      },
    },
    required: ['closingId'],
  },
...
interface GenerateNvPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateNvPurchaseAgreementInput,
  ...
  const data = {
    ...
    escrowCompany: closing.titleCompany,
  }

  const pdfBuffer = await generateNvPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `nv-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Nevada Purchase Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Nevada Residential Purchase Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Nevada Residential Purchase Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
