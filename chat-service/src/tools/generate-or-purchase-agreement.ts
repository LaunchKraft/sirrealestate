...
import { generateOrPurchaseAgreement } from '../forms/templates/or-purchase-agreement'
...
export const definition = {
  name: 'generate_or_purchase_agreement',
  description:
    'Generate the Oregon Residential Real Estate Sale Agreement (OREF-001) for an Oregon property purchase. ' +
    'Call this after offer terms are finalized for OR properties. ' +
...
interface GenerateOrPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateOrPurchaseAgreementInput,
  ...
  const pdfBuffer = await generateOrPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `or-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Oregon Sale Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Oregon Residential Real Estate Sale Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Oregon Sale Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
      ...
      'The seller must also provide the OREF 020 Seller\'s Property Disclosure Statement.',
  }
}
