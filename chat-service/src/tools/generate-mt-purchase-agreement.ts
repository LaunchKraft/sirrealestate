...
import { generateMtPurchaseAgreement } from '../forms/templates/mt-purchase-agreement'
...
export const definition = {
  name: 'generate_mt_purchase_agreement',
  description:
    'Generate the Montana Buy-Sell Agreement (MAR form) for a Montana property purchase. ' +
    'Call this after offer terms are finalized for MT properties. ' +
    'Ask the buyer whether the property has associated water rights — this is important in Montana.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      hasWaterRights: {
        type: 'boolean',
        description: 'Whether the property has associated water rights. Default false.',
      },
    },
    required: ['closingId'],
  },
...
interface GenerateMtPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateMtPurchaseAgreementInput,
  ...
  const data = {
    ...
    hasWaterRights: input.hasWaterRights ?? false,
    contingencies: {
  ...
  const pdfBuffer = await generateMtPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `mt-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Montana Buy-Sell Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Montana Buy-Sell Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Montana Buy-Sell Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
      'Please sign before submitting the offer to the seller\'s agent. ' +
      'Reminder: earnest money must be deposited within 2 business days of acceptance.',
  }
}
