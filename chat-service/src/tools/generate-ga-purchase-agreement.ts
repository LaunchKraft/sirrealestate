...
import { generateGaPurchaseAgreement } from '../forms/templates/ga-purchase-agreement'
...
export const definition = {
  name: 'generate_ga_purchase_agreement',
  description:
    'Generate the Georgia Purchase and Sale Agreement (GAR Form F20) for a Georgia property purchase. ' +
    'In Georgia, the "Binding Agreement Date" is the date the last party signs — all deadlines run from this date. ' +
    'EMD is due within 3 banking days of the Binding Agreement Date. ' +
    'Always confirm dueDiligenceDays (typically 10 business days) with the buyer before generating. ' +
    'Ask if the buyer has any special stipulations to include.',
  input_schema: {
    type: 'object',
    properties: {
      ...
      dueDiligenceDays: {
        type: 'number',
        description: 'Due diligence period in calendar days from Binding Agreement Date (default 10).',
      },
      specialStipulations: {
        type: 'array',
        description: 'Optional list of special stipulations to include in the agreement.',
        items: { type: 'string' },
      },
    },
    required: ['closingId'],
...
interface GenerateGaPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateGaPurchaseAgreementInput,
  ...
  const data = {
    ...
    dueDiligenceDays: input.dueDiligenceDays ?? 10,
    ...
    specialStipulations: input.specialStipulations,
    contingencies: {
  ...
  const pdfBuffer = await generateGaPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `ga-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Georgia Purchase and Sale Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Georgia Purchase and Sale Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Georgia Purchase and Sale Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
      'Earnest money is due within 3 banking days of the Binding Agreement Date (the date both parties have signed). ' +
