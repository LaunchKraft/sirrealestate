...
import { generateWaPurchaseAgreement } from '../forms/templates/wa-purchase-agreement'
...
export const definition = {
  name: 'generate_wa_purchase_agreement',
  description:
    'Generate the Washington Residential Purchase and Sale Agreement (NWMLS Form 21) for a Washington property purchase. ' +
    'Call this after offer terms are finalized for WA properties. ' +
    'In Washington, all deadlines run from the "mutual acceptance date" — when both parties have signed. ' +
    'EMD is due within 2 business days of mutual acceptance. ' +
  ...
  input_schema: {
    ...
    properties: {
      ...
      inspectionDays: {
        type: 'number',
        description: 'Number of business days for the inspection period (default 10). Runs from mutual acceptance.',
      },
    },
    required: ['closingId'],
  },
...
interface GenerateWaPurchaseAgreementInput {
  closingId: string
...
export async function execute(
  userId: string,
  input: GenerateWaPurchaseAgreementInput,
  ...
  const pdfBuffer = await generateWaPurchaseAgreement(data)

  const documentId = randomUUID()
  const fileName = `wa-purchase-agreement-${closing.closingId}.pdf`
  ...
  const title = `Washington Purchase and Sale Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Washington Residential Purchase and Sale Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Washington Purchase and Sale Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
      'Please sign before submitting the offer to the seller\'s agent. ' +
      'Reminder: earnest money must be deposited within 2 business days of mutual acceptance. ' +
      'The seller must also provide the Seller Disclosure Statement (Form 17) per RCW 64.06.013.',
  }
}
