...
import { generateNvAddendum } from '../forms/templates/nv-addendum'
...
export const definition = {
  name: 'generate_nv_addendum',
  description:
    'Generate an NVAR Addendum to Purchase Agreement for a Nevada closing. ' +
    'Use this during or after the Due Diligence Period to document agreed repairs, seller credits, or other contract modifications. ' +
  ...
  input_schema: {
    ...
    properties: {
      ...
      amendments: {
        type: 'array',
        description: 'List of agreed modifications to the contract (minimum 1 item).',
        items: {
          type: 'object',
          properties: {
            description: {
              type: 'string',
              description: 'Description of the modification, e.g. "Seller to repair HVAC by closing" or "Seller to provide $3,000 credit at closing".',
            },
          },
...
interface GenerateNvAddendumInput {
  closingId: string
...
export async function execute(
  ...
  const pdfBuffer = await generateNvAddendum(data)

  const documentId = randomUUID()
  const fileName = `nv-addendum-${closing.closingId}.pdf`
  ...
  const title = `Nevada Addendum to Purchase Agreement — ${closing.listingAddress}`
  ...
  const { signatureRequestId } = await signer.createSignatureRequest({
    ...
    subject: title,
    message:
      'Your Nevada Addendum to Purchase Agreement is ready for review and signature. ' +
  ...
  return {
    documentId,
    message:
      `Nevada Addendum to Purchase Agreement generated and sent to ${buyerEmails} for signing via Dropbox Sign. ` +
